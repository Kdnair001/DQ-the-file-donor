...
class Database:
    ...
    async def get_all_users(self):
        users_list = (await (self.col.find({})).to_list(length=None)) + (
            await (self.col2.find({})).to_list(length=None)
        )
        return users_list
    ...
    async def disable_chat(self, chat_id, reason="No Reason"):
        """
        Keep method for compatibility, but you won't really need it
        now that cleanup uses delete_chat().
        """
        chat_status = dict(
            is_disabled=True,
            reason=reason,
    ...
    async def delete_chat(self, chat_id):
        """Completely remove chat from both primary & secondary DB."""
        try:
            await self.grp.delete_many({"id": int(chat_id)})
            await self.grp2.delete_many({"id": int(chat_id)})
        except Exception:
            # Silent fail – we don't want cleanup to crash because of DB
            pass
    ...
    async def get_all_chats(self):
        return (await (self.grp.find({})).to_list(length=None)) + (
            await (self.grp2.find({})).to_list(length=None)
        )
